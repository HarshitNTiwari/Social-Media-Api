...
def unfollow_user(user_id, unfollow_id):
	if user_id != follow_id:
		...
		if follow_id in followings:
			users_collection.update_one({'_id': user_id}, {'$pull':{'following': follow_id}})
			users_collection.update_one({'_id': follow_id}, {'$pull':{'follower': user_id}})
			return "OK"
		return "Unfollow failed : follow_id not in following list of user_id"
	return "Unfollow failed : follow_id same as user_id"
...
def add_post(post):
	if post.title and post.desc:
		posts_collection.insert_one({'_id': post.post_id, 'created_by': post.created_by, 'title': post.title, 'desc': post.desc, 'created_at': post.created_at})
		return "OK"
	else:
		return "Title or Desc not found"
...
def get_post(post_id):
	post = posts_collection.find_one({'_id': post_id})
	if post:
		return Post(post['post_id'], post['created_by'], post['title'], post['desc'], post['created_at'], post['comment'], post['likes_count'])
	return "Post ID not found"
